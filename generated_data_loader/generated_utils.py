...
def getWordsInCrop(name, x0, y0, x1, y1):
    delimiter = ';'
    scope_tolerance = 256 * 0.03 # normally try 0.05
    # print('x0: ' + str(x0) + ' y0: ' + str(y0) + 'x1: ' + str(x1) + ' y1: ' + str(y1))
    csvCrop = open('data/generated/csv-crop/' + name + '-crop.csv', 'w+')
    word = ""
    pre = ""
    post = ""
    y_scope_out = False
    ...
    with open('data/generated/csv/' + name + '.csv') as csvfile:
...
def cropWords(image, name):
    word_array = []
    info_array = []
    with open('data/generated/csv-crop/' + name + '-crop.csv') as csvfile:
        reader = csv.DictReader(csvfile, delimiter=';')
        for row in reader:
            # print(row['y0'])
